...
class DriftSignalProtocolStore implements SignalProtocolStore {
  final SigmaDatabase _db;
  final KeyStore _keyStore;
  
  IdentityKeyPair? _cachedIdentityKeyPair;
  int? _cachedRegistrationId;

  DriftSignalProtocolStore(this._db, this._keyStore);
  ...
  Future<void> _ensureLocalIdentity() async {
    ...
    final serializedIdentity = await _keyStore.read(_keyStore.identityKeyPairKey);
    final registrationIdStr = await _keyStore.read(_keyStore.registrationIdKey);
