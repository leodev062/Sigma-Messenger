...
type WhisperServerConfiguration struct {
	...
	Bots      BotsConfiguration           `yaml:"bots"`
	Redis     RedisConfiguration          `yaml:"redis"`
}

type BotsConfiguration struct {
	BotFatherUsername    string `yaml:"bot_father_username"`
	BotFatherName        string `yaml:"bot_father_name"`
	APIPublicBaseURL     string `yaml:"api_public_base_url"`
	AllowInsecureWebhook bool   `yaml:"allow_insecure_webhook"`
}

func (cfg BotsConfiguration) Resolved() BotsConfiguration {
	resolved := cfg
	if resolved.BotFatherUsername == "" {
		resolved.BotFatherUsername = "BotFather"
	}
	if resolved.BotFatherName == "" {
		resolved.BotFatherName = "BotFather"
	}
	if resolved.APIPublicBaseURL == "" {
		resolved.APIPublicBaseURL = "http://localhost:3000"
	}
	return resolved
}
...
type ServerConfiguration struct {
	Port      string                 `yaml:"port"`
	CORS      CORSConfiguration      `yaml:"cors,omitempty"`
...
type WebSocketConfiguration struct {
	...
	AllowTokenQueryParam   *bool `yaml:"allow_token_query_param,omitempty"`
	AllowLegacyUserIDQuery bool  `yaml:"allow_legacy_user_id_query,omitempty"`
}

func (cfg WebSocketConfiguration) Normalized() WebSocketConfiguration {
	normalized := cfg
	if normalized.MaxConnectionsPerUser <= 0 {
		normalized.MaxConnectionsPerUser = 5
	}
	return normalized
}

func (cfg WebSocketConfiguration) TokenQueryAllowed() bool {
	if cfg.AllowTokenQueryParam == nil {
		return true
	}
	return *cfg.AllowTokenQueryParam
}
...
func (cfg CORSConfiguration) ResolvedAllowOrigins() []string {
...
func LoadConfig(path string) (*WhisperServerConfiguration, error) {
	...
	cfg.Server.WebSocket = cfg.Server.WebSocket.Normalized()
...
func (cfg *WhisperServerConfiguration) Validate() error {
	...
	for _, origin := range cfg.Server.CORS.AllowOrigins {
		if strings.TrimSpace(origin) == "*" {
			return fmt.Errorf("server.cors.allow_origins must not include wildcard *; list explicit app origins")
		}
	}
