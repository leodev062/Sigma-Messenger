...
type OfflineMessenger interface {
	Deliver(recipientID, destinationType string, payload []byte, notify bool) error
}
...
func (s *MessageDispatchService) Dispatch(senderID, destinationID, destinationType string, payload []byte) error {
	return s.DispatchContext(context.Background(), senderID, destinationID, destinationType, payload)
}

func (s *MessageDispatchService) DispatchContext(ctx context.Context, senderID, destinationID, destinationType string, payload []byte) error {
	...
	// MRDA Compliance: Routing based on mandatory destinationType
	...
	case "USER", "GROUP", "CHANNEL":
		// Fall through to router logic which handles member fan-out
		...
		for _, target := range plan.Targets {
			...
			if err := s.offline.Deliver(targetID, destinationType, payload, !target.SkipPush); err != nil {
	...
	default:
		s.logger.Printf("dispatch error: unsupported destinationType=%s", destinationType)
