...
func (p *PushManager) SendWakeupPush(fcmToken string) error {
	...
	if err != nil {
		if messaging.IsRegistrationTokenNotRegistered(err) || strings.Contains(err.Error(), "Requested entity was not found") {
			return fmt.Errorf("FCM token unregistered: %w", err)
		}
		return err
	}
